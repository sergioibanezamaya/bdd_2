...
const body = [
  ...
  table([
    ...
    ['Frontend paciente', 'React 19 + Vite (build separado)'],
  ...
  p('Es un monorepo con tres carpetas independientes: /backend, /frontend, /frontend-paciente.'),
  p(''),
  ...
  code('┌────────────────────────┐         ┌────────────────────────┐'),
  code('│ FRONTEND ODONTÓLOGO    │         │ FRONTEND PACIENTE      │'),
  code('│ React + Vite           │         │ React + Vite           │'),
  code('│ /pacientes  /turnos    │  HTTP   │ /portal (login)        │'),
  code('│ /agenda  /consultas    │ ◄────►  │ /home  /reservar       │'),
  code('└──────────┬─────────────┘         └──────────┬──────────────┘'),
  ...
  bullet('¿Por qué dos frontends? Porque tienen usuarios distintos, UIs distintas y se deployan por separado. Comparten el mismo backend.'),
